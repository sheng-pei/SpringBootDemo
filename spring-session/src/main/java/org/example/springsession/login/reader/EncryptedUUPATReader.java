...
import org.example.springsession.login.reader.random.LTRandomFilter;
...
public class EncryptedUUPATReader<H extends HttpSecurityBuilder<H>>
    ...
    public void addFilters(H builder) {
        if (randomManager != null) {
            LTRandomFilter filter = new LTRandomFilter();
            filter.setRandomManager(randomManager);
            builder.addFilterBefore(filter, ForceEagerSessionCreationFilter.class);
        }
    }
    ...
    private byte[] body(HttpServletRequest request) throws
        ...
        try {
            ...
            throw new HttpMediaTypeNotSupportedException(
                    ex.getMessage(), SUPPORTED_MEDIA_TYPES);
        }
    ...
    private byte[] decrypt(byte[] body) {
        if (keyCenter.ec() == null || keyCenter.ec().pri() == null) {
            throw new LoginStringDecryptedException("Private key not found.");
        }

        try {
            ECPrivateKey privateKey = BCECUtils.privateKey(keyCenter.ec().pri());
            return ECUtils.decrypt(privateKey, body);
        } catch (Throwable t) {
